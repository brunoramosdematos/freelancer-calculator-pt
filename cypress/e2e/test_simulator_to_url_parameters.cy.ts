...
describe("pass assessment scenario and dependents to url parameters", () => {
  it("selects each assessment scenario and updates the calculation", () => {
  ...
  it("reset clears new query parameters and returns controls to defaults", () => {
    cy.visit("/#/?income=60000&currentTaxRankYear=2024");
    cy.get('[data-cy="assessment-scenario-joint-single-income"]').check({
      force: true,
    });
...
describe("pass firstYear to url parameters", () => {
  it("successfully uses true firstYear from simulator", () => {
    cy.visit("/#/?income=50000"); // change URL to match your dev URL
    openAdvancedTaxSettings();
    cy.get('[data-cy="first-year"]').click();
